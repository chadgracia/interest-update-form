...
def stop_daily_updates(person_id: int, jwt: str) -> dict:
    ...
    contact = call_pipeline_api("GET", f"/people/{person_id}.json", jwt=jwt)
    contact_name = (contact.get("data") or {}).get("full_name", "client") if contact["status"] == 200 else "client"
    load_security_maps(jwt)
    send_email(
        CHAD_EMAIL,
        f"Daily updates stopped: {contact_name} (#{person_id})",
